...
conn_pg = psycopg2.connect("dbname='lcpl_16may' user=postgres password=postgres host='localhost' ")
pg_cursor = conn_pg.cursor()
pg_cursor.execute("select name from ir_module_module where state = 'installed'")
modules = pg_cursor.fetchall()

files_to_copy = [i[0] for i in modules]

source_path = '/opt/odoo9/openerp/lcpl_addons/' 
dest_path='/opt/odoo9/openerp/MMM/'
